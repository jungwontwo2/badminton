...
public interface MatchResultRepository extends JpaRepository<MatchResult,Long> {
    ...
    @Query("SELECT m From MatchResult m WHERE (m.winner1 = :user OR m.winner2 = :user OR m.loser1 = :user OR m.loser2 =:user) AND m.status ='AWAITING_OPPONENT' ORDER BY m.matchDate DESC")
